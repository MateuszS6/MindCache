...
@app.post("/summarise")
def summarise(input_text: InputText, request: Request, db: Session = Depends(get_db)): # Intended for private deployments: [, _: None = Depends(verify_api_key)]
...
@app.get("/history")
def history(limit: int = 20, db: Session = Depends(get_db)): # Intended for private deployments: [, _: None = Depends(verify_api_key)]
